...
class TrueFalseController extends GetxController {
  ...
  var selectedTrueFalse =
      'Select answer'.obs; // Initialize with 'Select answer'
  ...
  void MultideleteMcq(List<int> ids) async {
    ...
    for (int id in ids) {
      success = await QuestionRepository().deleteTrueFalse(id);
  ...
  void deleteTrueFalse(int id) async {
    isLoading(true);
    await QuestionRepository().deleteTrueFalse(id);
    isLoading(false);
    fetchData();
  }
  ...
  void manageTandF() async {
    ...
    var map = {
      ...
      'answer': (selectedTrueFalse.value == 'Select answer')
