...
class BlankController extends GetxController {
  ...
  late BlankDataSource blankDataSource;
  ...
  void fetchData() async {
    ...
    questionModel = await QuestionRepository().getBlank();
    blankDataSource =
  ...
  Future<void> processExcelData(File file, BuildContext context) async {
    ...
    for (var row in table!.rows) {
      ...
      print(map);
      var result = await QuestionRepository().addBlank(map);
      if (result == false) {
        Get.rawSnackbar(
  ...
  void deleteBlank(int id) async {
    isLoading(true);
    await QuestionRepository().deleteBlank(id);
    isLoading(false);
    fetchData();
  }
  ...
  void MultideleteMcq(List<int> ids) async {
    ...
    for (int id in ids) {
      success = await QuestionRepository().deleteBlank(id);
  ...
  void addBlank() async {
    ...
    if (result == true) {
      Get.rawSnackbar(
          message: 'Fill in the Blank Added Sucessfully!',
          backgroundColor: Colors.green);
    } else {
      Get.rawSnackbar(
          message: 'Fill in the Blank Added failed!',
          backgroundColor: Colors.redAccent);
    }
    fetchData();
    isAdding(false);
